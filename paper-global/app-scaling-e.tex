...
\begin{figure}[t]
  \centering
  \includegraphics[scale=0.7]{plots/scaling_e_zoom.labels.pdf}
  \caption{\textbf{Runtime scaling with divergence (zoomed)} (linear, synthetic,
    $n{=}10^4$, $10^6\bp$ total, $k{=}10$). The figure shows the same
    results as~\cref{fig:scaling-e}, but zoomed in to small runtimes where the
    scaling with $n$ degrades from linear to quadratic. $r{=}1$ and $r{=}2$
    indicate exact and inexact matches, respectively.}
