...
\begin{figure}[t]
    \centering
	\includegraphics[width=0.95\linewidth]{figures/seed-heuristic-tables}
	\caption[States exploration using the seed heuristic]{Exploration of
	$\AG[q]$, searching for a shortest path from the first to the last row using
	the \seedh. The table entry in the $i^\text{th}$ row (zero indexed) below
	node $v$ shows $g\st{v}{i}+h\st{v}{i}$, where $g\st{v}{i}$ is the shortest
	distance from any starting state $\st{u}{0}$ to $\st{v}{i}$.
	%
